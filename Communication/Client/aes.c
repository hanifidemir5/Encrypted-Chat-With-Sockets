...
void expandKey(unsigned char* expandedKey, unsigned char* key) {
    ...
    unsigned char t[4];
    int rconIteration = 1;
    size_t expandedKeySize = EXPANDED_KEY_SIZE;


    for (int i = 0; i < 16; i++)
        expandedKey[i] = key[i];
    currentSize += AES_BLOCK_SIZE;
    ...
    while (currentSize < expandedKeySize)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            expandedKey[currentSize] = expandedKey[currentSize - AES_BLOCK_SIZE] ^ t[i];
            currentSize++;
        }
...
void encryptMessage(unsigned char* input, unsigned char* output, unsigned char* key, unsigned char* counter)
{
    ...
    size_t num_blocks = (input_length + AES_BLOCK_SIZE) / AES_BLOCK_SIZE;
    int i, j = 0, l = 0, n, k;
    ...
    for (size_t i = 0; i < num_blocks; i++)
    {
        ...
        if (i == num_blocks - 1)
        {
            for (j = input_length % AES_BLOCK_SIZE; j < AES_BLOCK_SIZE; j++)
            {
                block[j] = 0x00;
            }
            block[j - 1] = '\0';
        ...
        aes_encrypt(counter, cipherCounterBlock, expandedKey, 16, 176);

        for (j = 0; j < AES_BLOCK_SIZE; j++)
        {
            encryptedBlock[j] = block[j] ^ cipherCounterBlock[j];
        }

        // Copy the encrypted block into the decrypted text array
        memcpy(output + block_start, encryptedBlock, block_length);
...
void decryptMessage(unsigned char* input, unsigned char* output, unsigned char* key, unsigned char* counter)
{
    ...
    for (i = 0; i < num_blocks; i++)
    {
        ...
        decryptedBlock[AES_BLOCK_SIZE - 1] = '\0';
