...
// Global variables
#define PORT 8080 // PORT number which server will run can be different or can be taken from user
#define MAX_CLIENTS 5 // It is what it say
#define BUFFER_SIZE 1024 // Max byte length which server will process for one message
unsigned char key[BUFFER_SIZE]; // Private key, can be declared every time server runs
int clientCount = 0;
...
// Function to handle communication with a client
    unsigned __stdcall ClientThread(void* arg) 
    {
        ...
        // Send key to user to use for all messages
        ...
        while (1) 
        {
            ...
            // The message which server takes
            ...
            if (buffer[0] == '@') 
            {
                ...
                if (!recipientFound) 
                {
                    snprintf(response, BUFFER_SIZE, "Error: User '%s' not found.", username);
                    send(clientSocket, response, strlen(response), 0);
                }
            }
            else 
            {
                // Find out sender username length as a char in order to sent it to client
...
int main() 
{
    ...
    unsigned threadID;
    int i, n;
    // Take key from server for furter usage 
    while (1)
    {
        ...
        key[strcspn(key, "\n")] = '\0';
        // Check if it is less or more than 16
        if (strlen(key) > 16)
        {
